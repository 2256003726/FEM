...
class _RecordByIdState extends State<RecordById> {
  ...
  _getRecordList(int type) async{     
    if(Provider.of<SpotProvider>(context, listen: false).selectedSpot != null) {
      ...
      if(type == 0) {
        await request(servicePath['getReElecById'], formData: formData).then((val) {
          if(val != null) {
            if(page > val['pages']) {
              page --;
        ...
        await request(servicePath['getReTempById'], formData: formData).then((val) {     
          if(val != null) {
            if(page > val['pages']) {
              page --;
      ...
      child: Row(
        children: <Widget>[
          RaisedButton(
            ...
            child: Text("未处理", style: TextStyle(color: selected==0? Colors.white: Colors.black54),),
          ...
          RaisedButton(
            padding: EdgeInsets.all(5),
            child: Text("已处理", style: TextStyle(color: selected==1? Colors.white: Colors.black54),),
...
Widget reElec(RecordElecModel reElec) {
  ...
  return Container(
    ...
    padding: EdgeInsets.all(5),
      ...
      secondaryActions: state == '1'?<Widget>[
      IconSlideAction(
        caption: '已处理',
        color: Colors.blue,
        icon: Icons.book,
